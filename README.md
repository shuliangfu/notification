...
> 一个兼容 Deno 和 Bun 的通知库，提供多渠道通知发送、订阅管理、模板系统和发送队列功能
...
通知发送库，用于多渠道消息推送、订阅管理和通知模板，提供 Web Push、邮件、短信、Webhook 等多渠道发送能力。
...
// 验证签名
const isValid = await verifyWebhookSignature(payload, signature, "webhook-secret");
...
本库经过全面测试，所有 114 个测试用例均已通过。详细测试报告请查看 [TEST_REPORT.md](./TEST_REPORT.md)。
...
- **VAPID 密钥**：Web Push 需要生成 VAPID 密钥对，公钥用于浏览器订阅，私钥用于服务端发送
